...
class Arm_Device(object):
	...
	def __receive_data(self):
		# Read header bytes safely, return False when incomplete
		...
		ext_len_bytes = self.ser.read(1)
		if not ext_len_bytes:
			return False
		ext_len = ext_len_bytes[0]

		ext_type_bytes = self.ser.read(1)
	...
	def Arm_serial_servo_write_any(self, id, angle, time):
		if id != 0:
			...
			time_L = time & 0xFF
			try:
				cmd=[0xFF,0xFC,0x07,0x19,id &0xFF,value_H,value_L,time_H,time_L]
				checksum = sum(cmd,5) & 0xff
				cmd.append(checksum)
				self.ser.write(cmd)
	...
	def Arm_serial_servo_write6_array(self, joints, time):
		...
		try:
			...
			print('Arm_serial_servo_write6_array serial error')		 	
		except:
			print('Arm_serial_servo_write6 serial error')
	...
	def Arm_Buzzer_Off(self):
		try:
			...
			print('Arm_Buzzer_Off serial error')
	...
	def Arm_serial_servo_read_any(self, id):
		...
		try:
			...
			print('Arm_serial_servo_read_any serial error')
			return None
		# 传感器返回的是高位、低位组合，注意转换顺序
	...
	def Arm_voice_write(self):
		try:
			...
			print('Arm_voice_write serial error')
